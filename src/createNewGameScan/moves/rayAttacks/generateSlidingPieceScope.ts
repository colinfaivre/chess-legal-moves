...
export function generateSlidingPieceScope(
    ...
    const pieceScope = piecePositions.map((position) => {
        ...
        const northEastAttacks = getPositiveRayAttacks(board.allPieces, 'noEa', position);
        const northWestAttacks = getPositiveRayAttacks(board.allPieces, 'noWe', position);
        const southEastAttacks = getNegativeRayAttacks(board.allPieces, 'soEa', position);
        const southWestAttacks = getNegativeRayAttacks(board.allPieces, 'soWe', position);

        const bishopAttacks = northEastAttacks.or(northWestAttacks).or(southEastAttacks).or(southWestAttacks);
        const rookAttacks = northAttacks.or(eastAttacks).or(westAttacks).or(southAttacks)
...
export function getPieceCategory(pieceName: ISlidingPiece, hasToPlay: IColor): IPieceCategory {
